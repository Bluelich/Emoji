...
NSString *arc4randomString(){
    /*
     ...
     〇               1字    3007
     */
    unsigned int number = arc4random()%(0x9FCB - 0x4E00) + 0x4E00;
    NSData *data = [NSData dataWithBytes:&number length:2];
    NSString *str = [[NSString alloc]initWithData:data encoding:NSUnicodeStringEncoding];
    return str;
}
NSString *arc4randomEmoji(){
    static NSArray<NSArray<NSString *> *> *emojis = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *path = [NSBundle.mainBundle pathForResource:@"emoji" ofType:@"json"];
        NSData *data = [NSData dataWithContentsOfFile:path];
        NSError *error = nil;
        emojis = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:&error];
    });
    unsigned int idx = arc4random()%(emojis.count);
    NSArray<NSString *> *emojiSeq = emojis[idx];
    unsigned int number = 0;
    for (NSInteger i = emojiSeq.count - 1; i >= 0; i--) {
        NSScanner *scanner = [NSScanner scannerWithString:emojiSeq[i]];
        unsigned val = 0;
        [scanner scanHexInt:&val];
        number |= val << (8 * i);
    }
    NSData *data = [NSData dataWithBytes:&number length:emojiSeq.count];
    NSString *str = [[NSString alloc]initWithData:data encoding:NSUnicodeStringEncoding];
    return str;
}
